...
export default async function handler(
  ...
  switch (method) {
    case "GET":
      const { page, pageSize, payment_type, chapter_id, search, startDate, endDate, paid_for }: any =
        req.query;
      const parsedPage = parseInt(page) || 1;
      const parsedPageSize = parseInt(pageSize) || 10;
      const searchQuery = search ? `%${search}%` : "%";
      ...
      try {
        ...
        if (paid_for){
          if (paid_for == "current")
            whereClauses.push(`month(members.mf_end_date) = month(curdate()) AND year(members.mf_end_date) = year(curdate())`)
          else if (paid_for == "advance")
            whereClauses.push(`members.mf_end_date >= LAST_DAY(CURDATE()) + INTERVAL 1 DAY`)
        }
