      ...
      try {
        ...
        if (rows.length > 0) {
          ...
          if (rows[0].membership_status === primary_number) {
            ...
            const token = jwt.sign({ id: rows[0].id, role: rows[0].role }, process.env.SECRET_KEY || "", {
            ...
            res.status(200).json({ success: true, token: token, message: 'Logged In Successfully', userRole: userRole, memberDetails:memberDetails });
          } else {
              try {
