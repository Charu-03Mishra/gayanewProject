...
export default async function handler(req: any, res: any, next: Function) {
  ...
  switch (method) {
    case "GET":
      const { page, pageSize, role, search, startDate, endDate, expense_type, status, chapter_id }: any = req.query;
      ...
      try {
        ...
        const [rows]: any = await conn.query(query, [...queryParams, parsedPageSize, offset]);
      ...
      expenseUpload.single("document")(req, res, async (err) => {
        ...
        try {
          ...
          if (existingEntries.length > 0) {
            ...
            return res.status(200).json({ message: "An expense with the same Chapter ID, Date of Meeting, and Invoice Number already exists." });
          }
          await conn.query(
