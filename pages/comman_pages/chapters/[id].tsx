...
const Index = () => {
  ...
  const initialValues = {
    ...
    isLaunched: chapterData?.is_launched || false,
    coffeeTable: chapterData?.coffee_table || false,
    meetingDay: chapterData?.meeting_day || "",
    meetingTime: chapterData?.meeting_time || "",
    launchedDate: chapterData?.launched_date && moment(chapterData?.launched_date).format('YYYY-MM-DD') || "",
    region: chapterData?.region_id || "",
    state: chapterData?.state || "",
    country: chapterData?.country || "",
    weekly_meeting_fees: chapterData?.weekly_meeting_fees || "",
    opening_balance: chapterData?.opening_balance || "",
    kitty_balance: chapterData?.kitty_balance || ""
  };
  ...
  const fetchChapterData = async () => {
    if (typeof window !== "undefined" && window.localStorage) {
      try {
        const response = await axios.get(`/api/chapters/${Number(id)}`, config);
